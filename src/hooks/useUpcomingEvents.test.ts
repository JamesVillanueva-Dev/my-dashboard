...
const INTENT = 'gcal.view.connected';

function event(id: string, date: Date): CalendarEvent {
  const ev = toEvent(
    { id, summary: id, start: { dateTime: date.toISOString() } },
    'cal-1',
  );
...
describe('useUpcomingEvents', () => {
  ...
  describe('connect', () => {
    it('loads the agenda interactively, so consent may be requested', async () => {
      ...
      expect(fetchUpcoming).toHaveBeenCalledWith(true, expect.any(Number));
    ...
    it('reports loading while the fetch is in flight', async () => {
      const gate = deferred<CalendarEvent[]>();
      vi.mocked(fetchUpcoming).mockReturnValue(gate.promise);
      const { result } = renderHook(() => useUpcomingEvents());
    ...
    it('surfaces a refused consent without forgetting the user asked', async () => {
      const { result } = renderHook(() => useUpcomingEvents());
      await act(async () => result.current.connect());
  ...
  describe('refresh', () => {
    it('re-reads silently, without risking a popup', async () => {
      ...
      expect(fetchUpcoming).toHaveBeenCalledWith(false, expect.any(Number));
    });
    ...
    it('gives up quietly when the Google session has gone', async () => {
      const { result } = renderHook(() => useUpcomingEvents());
      await act(async () => result.current.connect());
      expect(JSON.parse(localStorage.getItem(INTENT)!)).toBe(true);
  ...
  describe('on mount', () => {
    it('reconnects silently when the user was connected before', async () => {
      ...
      await waitFor(() => expect(fetchUpcoming).toHaveBeenCalledWith(false, expect.any(Number)));
  ...
  describe('now', () => {
    it('advances once a minute, so "Now" moves on its own', async () => {
      vi.useFakeTimers();
      const { result } = renderHook(() => useUpcomingEvents());
      const started = result.current.now;

      await act(async () => {
        vi.advanceTimersByTime(60_000);
      });

      expect(result.current.now).toBeGreaterThanOrEqual(started);
