...
criteria; a story marked **(done)** is implemented today, one marked
**(planned)** is a follow-up. Individual criteria that are still outstanding are
prefixed **Planned —**.
...
- Planned — Sync with my Google Docs (requires Google sign-in).
...
**Acceptance criteria**
- A row of bookmarks with site favicons, in the nav bar rather than a panel —
  navigation should not be something you scroll to find.
...
### US-10 — Manage widgets (movable & removable) (done)
...
### US-11 — Works offline-first for my data (done)
